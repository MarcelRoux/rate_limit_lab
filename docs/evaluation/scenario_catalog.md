...
| S_M5_4_OBSERVABILITY_UI_MVP | M5.4 (optional) | AT-052, AT-053, AT-054, AT-055, AT-056 | Validate optional observability contracts and report evidence linkage | observability-enabled runtime + config/provisioning checks (+ live probes when `EVAL_OBS_LIVE=1`) | metrics endpoint contract, scrape/dashboard contracts, and report evidence are present; live mode additionally verifies containerized Prometheus target `UP` and Grafana dashboard API discoverability | observability evidence section with links to config/provisioning/report artifacts (and live probe artifacts when enabled) | ready |
