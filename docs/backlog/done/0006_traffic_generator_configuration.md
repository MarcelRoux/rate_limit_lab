...
- [x] Traffic binary can load profile settings from a TOML config file.
- [x] Key mode and key distribution can be configured without code edits.
- [x] Invalid config values fail fast with actionable error messages.
- [x] Existing default behavior remains available when no config file is provided.
...
- Config-driven execution implemented in `crates/traffic_rest/src/bin/rest_traffic.rs` with `--config` support.
- Typed config model and validation implemented in `crates/traffic_rest/src/config.rs`.
- Example smoke profile added at `configs/traffic_rest/smoke.toml`.
- Default behavior without config remains aligned with prior hardcoded values.
