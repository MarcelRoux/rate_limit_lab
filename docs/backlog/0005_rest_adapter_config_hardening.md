...
- `crates/adapters/rest/src/bin/rest_client.rs`
...
## Acceptance criteria
- [ ] Build fails when zero or multiple REST limiter family features are enabled.
- [ ] REST server supports config-file driven parameterization with clear defaults.
- [ ] In-memory and distributed variants each have a working example config file.
- [ ] Startup logs expose effective runtime settings needed for experiment traceability.
